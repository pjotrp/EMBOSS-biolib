...
int scanned;
  ...
  if(ctgry){
    ...
    if(auto_){
     ...
     probcat    = Malloc(1*sizeof(double));
     rate[0]    = 1.0;
     probcat[0] = 1.0;
   }

  /*  outgropt = ajAcdGetBoolean("og");
  if(outgropt)
...
void emboss_inputdata(void)
{
 ...
 weightsum = sites;
  ...
  for(i=0;i<spp;i++){
    ...
    strncpy(naym[i],ajStrGetPtr(ajSeqsetGetseqNameS(seqset, i)),ilen);
     ...
     /*void doinit()*/
{
  ...
  /*  getnums();
      getoptions();*/
  emboss_getoptions("ednamlk",argc,argv);
  emboss_getnums();
