...
static DrvOpt    win3_options[] = {
    ...
    {"hwnd", DRV_INT, 0, &hwnd, "Windows HWND handle (not supposed to be given as a command line argument)"},
    {"buffered", DRV_INT, 0, &buffered, "Sets buffered operation"},
#ifdef HAVE_FREETYPE
    {"text", DRV_INT, 0, &freetype, "Use driver text (FreeType)"},
    {"smooth", DRV_INT, 0, &smooth_text, "Turn text smoothing on (1) or off (0)"},
    {"save", DRV_INT, 0, &save_reg, "Save defaults to registary"},
#endif
    {NULL, DRV_INT, 0, NULL, NULL}
};

LRESULT __declspec(dllexport) CALLBACK PlPlotWndProc (HWND hwnd,UINT message,UINT wParam,LONG lParam);
 ...
 */
LRESULT __declspec(dllexport) CALLBACK PlPlotWndProc (HWND hwnd, UINT message,
