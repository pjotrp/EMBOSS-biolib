...
typedef struct AjSDomConts
{
  ...
  AjPStr *res_pos2;      /* Array of residue positions in complete protein coordinate file - exist as strings */
...
int main(ajint argc, char **argv)
{
  ...
  AjPFile  het_fptr=NULL;               /* Pointer to dictionary of heterogens file (output from Dichet.c) */
  ...
  AjPInt   siz_heterogens=NULL;        /* Integer array with numbers of elements in list_heterogens */
  AjPList  list_heterogens=NULL;       /* List of heterogen Atom-arrays (each for each heterogen in
					 current Pdb object */
  ajint    n_heterogens=0;             /* no. of heterogens/arrays in current Pdb object */
    
  /* Create list of scop identifiers for domains in pdb object */
  AjPList  list_pdbscopids=NULL;       /* List to hold SCOP domain identifiers for domains in pdb object */
  AjPStr   scopid_tmp=NULL;            /* Temprorary pointer for ajListPop to free memory for list_pdbscopid */
  ...
  AjPList  list_domfnames=NULL;        /* List of domain coordinate filenames for domains in pdb object */
  AjPStr   domfname_tmp=NULL;          /* Temprorary pointer for ajListPop to free memory for list_domfnames */
    
  /* Open domain coordinate file and create list of domain pdb objects */
  AjIList  iter_domfnames=NULL;        /* Iterator for list of domain coordinate filenames */
  AjPStr   pdbscop_fname=NULL;         /* Pointer to current domain coordinate filename */
  ...
  AjPList  pdbdomList=NULL;             /* List of domain pdb objects for domains in current protein coordinate file */
  ...
  AjPPdb   pdbdom_ptr=NULL;            /* Pointer to current domain pdb object in pdbdomList - also used for freeing memory*/
  ...
  /* SPECIFY EXTENSION FOR DOMAIN COORDINATE FILES - APPEND WITH '.' IF NOT ALREADY PRESENT */
  ...
  ajFileScan(prot, p_fname, &pdbfile_list, ajFalse, ajFalse, NULL, NULL, ajFalse, NULL); 
  ...
  while(ajListstrPop(pdbfile_list, &cp_file))
	...
	ajFmtPrint("CPDB FILE: %S (%d/%d)\n", cp_file, f_ctr++, f_num);
      ...
      fflush(stdout);
      ndomain=0;			/* Reset counter for the number of domains in this file */
      ...
      if(!HetTest(pdb))
	{
      ...
      n_heterogens=0;
      funky_CpdbListHeterogens(pdb, &list_heterogens, &siz_heterogens, &n_heterogens, logf);	
      
      l=ajListLength(list_heterogens);
      ajFmtPrintF(logf, "\tHET:%d", l);
      
      /* CREATE LIST OF SCOP IDENTIFIERS FOR SCOP DOMAINS OCCURRING IN CURRENT PDB OBJECT */
      ...
      while((pdbscop_fname=ajListIterNext(iter_domfnames))) 
	{
	  if((dom_fptr=ajFileNewIn(pdbscop_fname))==NULL)
	    {
	      ajFmtPrintS(&msg, "Could not open for reading: %S", pdbscop_fname);
	      ajWarn(ajStrStr(msg));
	      continue;
	    }
	  /* Write pdb object */
      ...
      /* CREATE LIST OF DOMAIN ATOM ARRAYS FOR DOMAINS IN CURRENT PROTEIN COORDINATE FILE */
      ...
      while((pdbdom_ptr=ajListIterNext(iter_pdbdom)))
	{
	  /* note: only one chain in domain coordinate file and so Chains[0] */
	    siz_atomarr=ajListToArray(pdbdom_ptr->Chains[0]->Atoms, (void ***) &atomarr); 
      ...
      /* CALCULATE CONTACTS BETWEEN ALL COMBINATIONS OF DOMAINS AND HETEROGENS IN CURRENT PDB OBJECT */
      funky_HeterogenContacts(dist_thresh, list_domains, list_pdbscopids, list_heterogens, siz_domains, siz_heterogens, vdw,  &dbase);
      ...
      while(ajListPop(pdbdomList, (void **) &pdbdom_ptr))
	{
      ...
      /* Free memory for the arrays themselves (but not the data that the array elements
  ...
  /* COMPLETE THE DBASE OBJECT - WRITE CONTACT DATA ARRAY FOR EACH HETEROGEN ENTRY IN THE OBJECT */
  for(i=0; i<(dbase)->n; i++)
  {
      (dbase)->entries[i]->no_dom=ajListToArray((dbase)->entries[i]->tmp, (void ***) &(dbase)->entries[i]->cont_data);	  
...
**
** @param [r] pdb             [AjPPdb]   Pointer to pdb object
** @param [r] list_allscop    [AjPList]  Pointer to SCOP list of SCOP classification objects  
...
**
** @param [r] list_domains       [AjPList]   Pointer to list of domain Atom arrays
** @param [r] list_heterogens    [AjPList]   Pointer to list of heterogen Atom arrays 
** @param [w] dbase              [AjPDbase*] Pointer to Dbase object
** 
** 
** @return no_contacts [ajint] No. of contacts
** @@
******************************************************************************/
...
AjBool      funky_HeterogenContacts(float dist_thresh, AjPList list_domains, AjPList list_pdbscopids, AjPList list_heterogens, AjPInt siz_domains, AjPInt siz_heterogens, AjPVdwall vdw, AjPDbase *dbase)
{
  ...
  AjPDomConts cont_dataTemp=NULL;	/* Temporary pointer for current heterogen:domain contact data */
  AjPStr   tempaa;                      /* Temp. pointer for amino acid code */
  AjPList aaTempList=NULL;		/* Temporary list for amino acid code */
  AjPStr  tempres_pos2=NULL;            /* NEW Temporary pointer for the original pdb residue positions */
  AjPList res_pos2TempList=NULL;        /* NEW Temporary list for residue positions from original pdb file */
  ...
  ajint   i=0;                          /* Counter for looping through domain atom array also used for looping through (*dbase)->entries */
      ...
      while((het_atm=(AjPAtom *)ajListIterNext(iter_het)))
	{
	  ...
	  for(i=0;i<dom_max;++i)
	      ...
	      if(dom_atm[i]->Idx==idx_tmp) 
		{
	      ...
	      /* 	      ajFmtPrint("\tDom %S Atom %d X:%.3f Y:%.3f Z:%.3f\n", scopidArr[scopctr-1], i, dom_atm[i]->X, dom_atm[i]->Y, dom_atm[i]->Z); */
		
	      /* loop through current hetetrogen atoms array */
	      for(j=0;j<het_max;j++)
		{	
		  ajStrAssS(&(cont_dataTemp)->scop_name, scopidArr[scopctr-1]);		  
		  /* Check with jon that the group numbers have been assigned correctly here, temp can be used in place of het_atm[j]->Id3 for diagnostics */
		  ajFmtPrintS(&temp, "Heterogen %S Chain %d Group %d", het_atm[j]->Id3, het_atm[j]->Chn, het_atm[j]->Gpn);
		   
		  ajStrAssS(&(cont_dataTemp)->het_name, het_atm[j]->Id3);
		  if(ajXyzInContact(dom_atm[i], het_atm[j], dist_thresh, vdw))
		    {
		      
		      /* ajFmtPrint("\tCONTACT: Domain %S atm %d Res. %d -- Heterogen %S Chain %d Group %d atm %d\n", scopidArr[scopctr-1], i, dom_atm[i]->Idx, het_atm[j]->Id3, het_atm[j]->Chn, het_atm[j]->Gpn, j); */		      
	  ...
	  for(j=0;j<(dbase)->entries[i]->no_dom; j++)
	      ...
	      if(dbase->entries[i]->cont_data[j]->no_keyres > 0)
		{
		  ...
		  for(k=0; k< dbase->entries[i]->cont_data[j]->no_keyres; k++)
		    {
		      ajFmtPrintF(funky_out, "RE   %S %d %S\n",  dbase->entries[i]->cont_data[j]->aa_code[k], ajIntGet(dbase->entries[i]->cont_data[j]->res_pos, k), dbase->entries[i]->cont_data[j]->res_pos2[k]); 
		    }
		  ajFmtPrintF(funky_out, "XX\n");
