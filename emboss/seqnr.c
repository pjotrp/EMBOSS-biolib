...
**
SEQNR DOCUMENTATION
seqnr reads a scop families file and a validation file generated by the EMBOSS
application seqsort and writes processed scop families and validation files.
For processing, it requires a structure-based sequence alignment file 
('alignment' or 'alignment file' hereon) for each family in the scop families 
file.

Each family in the scop families file is processed so that redundant hits are
removed.  Hits derived from sequences in the corresponding alignment are 
considered when calculating redundancy.  The processed file can then be used 
to build an extended alignment for each family by using the EMBOSS program 
alignwrap (which extends the alignment with sequences from the processed file).

The validation input file will contain hits that could not be uniquely assigned
to a single scop family.  It is processed so that all of the hits from the 
scop families input file plus hits corresponding to sequences from the
alignment files are incorporated. 

Note however that the validation file must only contains hits with known accession 
numbers.  Therefore if an accession number could not be assigned to a hit from the 
alignment file, then that hit will NOT appear in the validation file, and a 
message to the log file will be written. Such cases will arise where the pdb code 
corresponding to a hit is not listed in the swissprot:pdb equivalence table, 
Epdbtosp.dat. (The hits in the scop families will normally have been found by a 
search against swissprot and therefore should all have accession numbers).

To avoid introducing duplicate hits in the validation file when incorporating 
hits from the alignment, any hits that have an identical accession number to an
alignment sequence and which overlap by at least 10 residues will NOT be written. 

Hits in the validation file are flagged as NON_REDUNDANT or REDUNDANT as 
appropriate.  They are then flagged as 'SEED' or left as 'HIT' depending on 
whether the file will be used to validate signatures generated from (i) 
structure-based sequence alignemnts (hits corresponding to sequences 
from the alignment files only are marked up as 'SEED') or (ii) extended alignments 
(all hits in the scop families output file are marked up as 'SEED').

Caveat - It should be care must be taken when validating signatures from 
structure-based sequence alignments, as some families in the validation file will 
not contain the same number of SEED's as sequences in the alignment.
...
Each family is processed in turn:
  ...
  1.3  Query scop classification file & retrieve the corresponding scop domains 
  1.4  Construct a list of these domains.

  2.   Mark up hits for this family that should be removed from the scop families 
       file (i.e. those from the alignment file).

  3.   Calculate redundancy and remove unwanted hits
  3.1  Perform pairwise comparison of all hits in list and identify redundant 
       hits. 
...
Each family in the validation file is processed in turn:
...
static AjBool seqnr_GetScopFromAlign(AjPScopalg align, AjPScop *scop_arr, ajint dim, 
				    AjPList* list);

static AjBool seqnr_ScophitsAccToHitlist(AjPList in, AjPHitlist *out,   AjIList *iter);
...
/*JISON*/        keep        = ajIntNew();
        ...
        for(iter = ajListIter(famin_hits), i = 0;(tmp_hit = (AjPScophit)ajListIterNext(iter));i++)
        {
...
** @@
********************************************************************************/
static AjBool seqnr_GetScopFromAlign(AjPScopalg align, AjPScop *scop_arr, ajint scop_dim, 
				    AjPList* list)
{
...
** @@
******************************************************************************/
AjBool seqnr_ScophitsAccToHitlist(AjPList in, AjPHitlist *out, AjIList *iter)
{
	...
	*iter=NULL;
	...
	if(!scoptmp)
	{
	    ajWarn("List with no Scophits with Acc in seqnr_ScophitsAccToHitlist");
	    ajListIterFree(*iter);	
	    *iter=NULL;
	    return ajFalse;
	}
