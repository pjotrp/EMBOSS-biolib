...
static void OutputFloat (AjPFile outfile, float num, AjBool html, AjBool after);
static void OutputInt (AjPFile outfile, ajint num, AjBool html, AjBool after);
static void OutputStr (AjPFile outfile, AjPStr str, AjBool html,
	AjBool after, ajint minlength);
static void Compare(AjPSeq ref, AjPSeq seq, ajint **sub, AjPSeqCvt cvt,
	ajint *seqlength, ajint *alignlength, ajint *gaps, ajint *gapcount, 
	ajint *idcount, ajint *simcount, 
	ajint *difcount, float *change);
...
/* get the stats from the comparison to the reference sequence */
	Compare(ref, seq, sub, cvt, &seqlength, &alignlength, 
...
/* start table line */
	...
	if (dousa)
          OutputStr(outfile, usa, html, (dodesc || dochange ||
		dodifcount || dosimcount || doidcount || dogapcount || dogaps ||
		doseqlength || doalignlength || doname), 18);

	if (doname)
          OutputStr(outfile, name, html, (dodesc || dochange ||
		dodifcount || dosimcount || doidcount || dogapcount || dogaps ||
		doseqlength || doalignlength), 14);

	if (doseqlength)
          OutputInt(outfile, seqlength, html, (dodesc || dochange ||
		dodifcount || dosimcount || doidcount || dogapcount || dogaps ||
		doalignlength));

	if (doalignlength)
          OutputInt(outfile, alignlength, html, (dodesc || dochange ||
		dodifcount || dosimcount || doidcount || dogapcount || dogaps));

	if (dogaps)
          OutputInt(outfile, gaps, html, (dodesc || dochange ||
		dodifcount || dosimcount || doidcount || dogapcount));

	if (dogapcount)
          OutputInt(outfile, gapcount, html, (dodesc || dochange ||
		dodifcount || dosimcount || doidcount));

	if (doidcount)
          OutputInt(outfile, idcount, html, (dodesc || dochange ||
		dodifcount || dosimcount));

	if (dosimcount)
          OutputInt(outfile, simcount, html, (dodesc || dochange ||
		dodifcount));

	if (dodifcount)
          OutputInt(outfile, difcount, html, (dodesc || dochange));

	if (dochange)
          OutputFloat(outfile, change, html, dodesc);

	if (dodesc)
	  OutputStr(outfile, desc, html, ajFalse, NOLIMIT);
...
Try to fit the name in 'minlength' spaces, else just add a TAB after it
*/
