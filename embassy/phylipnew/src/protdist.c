...
void   protdist_inputdata(AjPSeqset);
void   doinput(void);
void   code(void);
...
{ /* input the information on the options */
  ...
  if ((ctgry && categs > 1) && (firstset || !justwts)) {
    inputcategsstr(phyloratecat->Str[0], 0, chars,
		   category, categs, "ProtDist");
    if (printdata)
...
{ /* reads the input data */
  ...
  inputoptions();
  protdist_inputdata(seqsets[ith-1]);
  if (!ctgry) {
    categs = 1;
    rate[0] = 1.0;
  }
...
#endif
  init(argc, argv);
  emboss_getoptions("fprotdist",argc,argv);
