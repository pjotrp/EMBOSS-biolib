 ...
 */
static void 
hmmer_getline(struct ReadSeqVars *V)
{
  char *cp;
...
readPIR(struct ReadSeqVars *V)
{
  ...
  hmmer_getline(V);			/* skip next line, coords */
...
readPearson(struct ReadSeqVars *V)
{
  ...
  while (V->longline && ! feof(V->f)) hmmer_getline(V);
...
readZuker(struct ReadSeqVars *V)
{
  ...
  hmmer_getline(V);  /*s == "seqLen seqid string..."*/
...
readSquid(struct ReadSeqVars *V)
{
  ...
  while (strncmp(V->sbuffer, "NAM ", 4) != 0) hmmer_getline(V);
