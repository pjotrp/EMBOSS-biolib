  ...
  while (tmp_block!=NULL) {
     if (tmp_block->to1) {
         ...
         if (file_type==EST_GEN) 
            (void)fprintf(SIM4_OUTDEV,"%d-%d  (%d-%d)   %d%%",
         ...
         if (((tmp_block1=tmp_block->next_exon)!=NULL) && tmp_block1->to1)
              switch (tmp_block->ori) {
                 case 'C':  (void)fprintf(SIM4_OUTDEV," <-\n");
                            break;
                 case 'E':  (void)fprintf(SIM4_OUTDEV," ==\n");
                            break;
                 case 'G':  (void)fprintf(SIM4_OUTDEV," ->\n");
                            break;
                 case 'N':  (void)fprintf(SIM4_OUTDEV," --\n");
                            break;
...
{
   int f1, f2=0, t1, t2, ot1=0, ot2=0, xto, xfrom;
...
static void print_introns(Intron_ptr intron_list) 
{
  ...
  while (ep!=NULL) {
     (void)fprintf(SIM4_OUTDEV,"genome: %6ld - %-6ld    cDNA: %5ld - %-5ld,  l: %-5d,  o: %c \n",
...
static void pluri_align(int *dist_ptr,int *num_matches,Exon *lblock,struct edit_script_list **Aligns)
{
   ...
   *dist_ptr = ali_dist = 0;
   ...
   while (((tmp_block1=tmp_block->next_exon)!=NULL) && tmp_block1->to1) {
         ...
         if (diff<0) {
             (void)fprintf(SIM4_OUTDEV,"The two sequences are not really similar.\n");
             (void)fprintf(SIM4_OUTDEV,"Please try an exact method.\n");
             exit(1);        
         }

#ifdef STATS
         if (diff>P*(tmp_block1->to2-tmp_block1->from2+1))
              (void)fprintf(SIM4_OUTDEV,"Warning: Distance threshold on segment exceeded.\n");
#endif
