        ...
        while (tp != NULL) {
           if (tp->op_type == SUBSTITUTE) {
                ...
                while (k < tp->num) {
                        ...
                        if (count > 0) (void)fprintf(SIM4_OUTDEV,"copy %d\n", count);
                        if (k < tp->num) {
                                (void)fprintf(SIM4_OUTDEV,"replace %c by %c\n", seq1[i], seq2[j]);
                                ++i;
                                ++j;
                                ++k;
                        }
                ...
                /*
                if (tp->num > 1) (void)fprintf(SIM4_OUTDEV,"%d substitutions\n", tp->num);
                else (void)fprintf(SIM4_OUTDEV,"%d substitution\n", tp->num);
                */
           } else if (tp->op_type == INSERT) {
                if ((tp==head || tp->next==NULL) && (M <= N))
                        (void)fprintf(SIM4_OUTDEV,"skip (second sequence) %d\n", tp->num);
                else {
                        /*
                        (void)fprintf(SIM4_OUTDEV,"insert %d\n", tp->num);
                        */
                        (void)fprintf(SIM4_OUTDEV,"insert ");
                        if (tp->num<=10)
                            for (k=j; k<j+tp->num; ++k) (void)fprintf(SIM4_OUTDEV,"%c", seq2[k]);
                        else (void)fprintf(SIM4_OUTDEV," %d ",tp->num);
                        (void)fprintf(SIM4_OUTDEV,"\n");
                }
                j += tp->num;
           } else {     /* DEL */
                if ((tp==head || tp->next==NULL) && (M > N))
                        (void)fprintf(SIM4_OUTDEV,"skip (first sequence) %d\n", tp->num);
                else {
                        /*
                        (void)fprintf(SIM4_OUTDEV,"delete %d\n", tp->num);
                        */
                        (void)fprintf(SIM4_OUTDEV,"delete ");
                        if (tp->num <= 10)
                            for (k=i; k<i+tp->num; ++k)
                                 (void)fprintf(SIM4_OUTDEV,"%c", seq1[k]);
                        else
                            (void)fprintf(SIM4_OUTDEV,"%d ",tp->num);
                        (void)fprintf(SIM4_OUTDEV,"\n");
                }
                i += tp->num;
           }
...
void S2A(edit_script *head, int *S, int flag)
{
        ...
        while (tp != NULL) {
/*
        (void)fprintf(SIM4_OUTDEV,"tp->op_type=%d, tp->num=%d\n",tp->op_type, tp->num);
*/
           if (tp->op_type == SUBSTITUTE) {
...
{ 
  ...
  while (i < M || j < N) {
    ...
    if ((a >= ALINE+50) || ((i >= M) && (j >= N))) {
        *a = *b = *c = '\0';
/*MATHOG: CHANGED next line: All other sequence numbering is from 1 */
        (void)fprintf(SIM4_OUTDEV,"\n%7d ",1 + 50*lines++);
...
static int rsnake(int k, int x, int startx, int starty, int M)
{
        ...
        if ((startx<0) || (starty<0))
            (void)fprintf(SIM4_OUTDEV,"TROUBLE!!! startx:  %5d,  starty:  %5d\n",startx, starty);
        if ((x>M) || (x+k>N))
             (void)fprintf(SIM4_OUTDEV,"TROUBLE!!! x:  %5d,  y:  %5d\n",x,x+k);
