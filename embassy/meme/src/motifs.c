...
)
{
  ...
  */
  ...
  while ( (i = fscanf(fptr, " MOTIF %s %d", motif[nmotifs].name,
    ...
    if (nmotifs == NMOTIFS) {
      printf("Too many known motifs (> NMOTIFS) in file %s!\n", filename);
      printf("Recompile with larger value of NMOTIFS.\n");
      exit(1);
    }
    ...
    while ( (i = fscanf(fptr, "%d %d ", &seq, &off) ) == 2) {
      ...
      if (!hash_lookup(seq_name, 0, ht_seq_names)) { 
  ...
  if (i != EOF) {
    printf("Error reading motif file %s\n", filename);
    exit(1);
  }

  return nmotifs;
}
/* $Header: /home/repository/emboss/emboss/emboss/embassy/meme/src/motifs.c,v 1.1 2000/11/05 21:47:55 ajb Exp $ */
