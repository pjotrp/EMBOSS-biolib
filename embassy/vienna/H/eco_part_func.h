...
extern struct ConcEnt  *get_concentrations(double FEAB, double FEAA, double FEBB, double FEA, double FEB, double * startconc, AjPFile outf);

extern struct plist *get_plist(struct plist *pl, int length, double cut_off);

extern struct plist *get_mfe_plist(struct plist *pl);
extern int make_probsum(int length, char *name, AjPFile outf);
