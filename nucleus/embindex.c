...
static EmbPBtpage btreeSplitPriLeaf(EmbPBtcache cache, EmbPBtpage spage);
...
static EmbPPriBucket btreePriBucketNew(ajint n);
static void          btreePriBucketDel(EmbPPriBucket *thys);
static EmbPPriBucket btreeReadPriBucket(EmbPBtcache cache, ajlong pageno);
static void          btreeWritePriBucket(EmbPBtcache cache,
					 const EmbPPriBucket bucket,
					 ajlong pageno);
static void          btreeAddToPriBucket(EmbPBtcache cache, ajlong pageno,
					 const EmbPBtPri pri);
static ajint         btreeNumInPriBucket(EmbPBtcache cache, ajlong pageno);
static ajint         btreeKeywordCompare(const void *a, const void *b);
static AjBool        btreeReorderPriBuckets(EmbPBtcache cache,
					    EmbPBtpage leaf);
void                 embBtreeInsertKeyword(EmbPBtcache cache,
					   const EmbPBtPri pri);



static void btreeWriteSecBucket(EmbPBtcache cache, const EmbPSecBucket bucket,
				ajlong pageno);
static EmbPSecBucket btreeReadSecBucket(EmbPBtcache cache, ajlong pageno);
static ajint btreeKeywordIdCompare(const void *a, const void *b);
static EmbPBtpage btreeSplitSecLeaf(EmbPBtcache cache, EmbPBtpage spage);

static EmbPSecBucket btreeSecBucketNew(ajint n);
static void btreeSecBucketDel(EmbPSecBucket *thys);
static void btreeAddToSecBucket(EmbPBtcache cache, ajlong pageno,
				const AjPStr id);
static AjBool btreeReorderSecBuckets(EmbPBtcache cache, EmbPBtpage leaf);
#if 0
static void btreeInsertIdOnly(EmbPBtcache cache, EmbPBtPri pri);
#endif
static void btreeSplitRootSec(EmbPBtcache cache);
static ajint btreeNumInSecBucket(EmbPBtcache cache, ajlong pageno);
static void  btreeInsertKeySec(EmbPBtcache cache, EmbPBtpage page,
			       const AjPStr key, ajlong less,
			       ajlong greater);
static ajlong btreeInsertShiftSec(EmbPBtcache cache, EmbPBtpage *retpage,
				  const char *key);
static void btreeKeyShiftSec(EmbPBtcache cache, EmbPBtpage tpage);
static void btreeInsertNonFullSec(EmbPBtcache cache, EmbPBtpage page,
				  const AjPStr key, ajlong less,
				  ajlong greater);
...
void embBtreeCreateRootNode(EmbPBtcache cache, ajlong rootpage)
{
    ...
    page = embBtreeCacheWrite(cache,rootpage);
    ...
    page->dirty = BT_DIRTY;
    embBtreeCacheSync(cache,rootpage);
    page->dirty = BT_LOCK;
...
**
** @param [w] thys [EmbPPriBucket*] bucket
**
** @return [void]
** @@
******************************************************************************/
...
**
**
** @return [EmbPBtPri] Index ID object
** @@
******************************************************************************/
...
**
** @param [w] thys [EmbPBtPri*] index keyword primary object
**
** @return [void]
** @@
******************************************************************************/
...
**
** @param [u] cache [EmbPBtcache] cache
** @param [r] bucket [const EmbPPriBucket] bucket
** @param [r] pageno [ajlong] page number
**
** @return [void]
** @@
...
static void btreeWritePriBucket(EmbPBtcache cache, const EmbPPriBucket bucket,
				ajlong pageno)
{
    EmbPBtpage page     = NULL;
    EmbPBtpage lpage    = NULL;
...
** @param [u] cache [EmbPBtcache] cache
** @param [r] pageno [ajlong] page number of bucket
** @param [r] pri [const EmbPBtPri] ID info
**
** @return [void]
** @@
******************************************************************************/
				...
				const EmbPBtPri pri)
{
    ...
    /* Reading a bucket always gives one extra ID position */
    bucket->codes[nentries] = embBtreePriNew();
	...
	embBtreeCreateRootNode(cache,secrootpage);
	cache->secrootblock = secrootpage;
	page = embBtreeCacheWrite(cache,secrootpage);
	page->dirty = BT_DIRTY;
	embBtreeCacheSync(cache,secrootpage);
	page->dirty = BT_LOCK;
	...
	page = embBtreeCacheWrite(cache,cache->secrootblock);
	page->dirty = BT_LOCK;
	buf = page->buf;
	GBT_RIGHT(buf, &right);
    ...
    embBtreeInsertSecId(cache, pri->id);
...
static ajint btreeKeywordCompare(const void *a, const void *b)
{
    return strcmp((*(EmbPBtPri*)a)->keyword->Ptr,
		  (*(EmbPBtPri*)b)->keyword->Ptr);
}
...
** @@
******************************************************************************/
static AjBool btreeReorderPriBuckets(EmbPBtcache cache, EmbPBtpage leaf)
{
    ajint nkeys = 0;
    EmbPPriBucket *buckets = NULL;
    unsigned char *lbuf = NULL;
    AjPStr *keys        = NULL;
    ajlong *ptrs        = NULL;
    ajlong *overflows   = NULL;
...
/* @func embBtreeInsertKeyword *********************************************
**
** Insert a keyword structure into the tree
**
** @param [u] cache [EmbPBtcache] cache
** @param [r] pri [const EmbPBtPri] Id object
**
** @return [void] pointer to a page
** @@
******************************************************************************/

void embBtreeInsertKeyword(EmbPBtcache cache, const EmbPBtPri pri)
{
    EmbPBtpage spage   = NULL;
    EmbPBtpage page    = NULL;
    EmbPBtpage parent  = NULL;
    AjPStr key         = NULL;
    const char *ckey      = NULL;
    EmbPPriBucket lbucket = NULL;
    EmbPPriBucket rbucket = NULL;
    ajlong lblockno = 0L;
    ajlong rblockno = 0L;
    ajlong blockno  = 0L;
    ajlong shift    = 0L;
    ...
    EmbPBtPri exists = NULL;

    
    ajDebug("In embBtreeInsertKeyword %S %S\n",pri->keyword,pri->id);

    /* Only insert a primary key if that key doesn't exist */
    exists = embBtreePriFromKeyword(cache,MAJSTRSTR(pri->keyword));
    if(exists)
    {
...
** @param [r] key [const char *] key
**
** @return [EmbPBtPri] pointer to an Pri structure or NULL if not found
** @@
******************************************************************************/
...
EmbPBtPri embBtreePriFromKeyword(EmbPBtcache cache, const char *key)
{
    ...
    ajDebug("embBtreePriFromKeyword\n");
...
** @param [r] pageno [ajlong] page number
**
** @return [EmbPSecBucket] bucket
** @@
******************************************************************************/

static EmbPSecBucket btreeReadSecBucket(EmbPBtcache cache, ajlong pageno)
{
    EmbPSecBucket bucket   = NULL;
    EmbPBtpage page     = NULL;
    EmbPBtpage lpage    = NULL;
    unsigned char *buf  = NULL;
    unsigned char *kptr = NULL;
...
**
** @param [u] cache [EmbPBtcache] cache
** @param [r] bucket [const EmbPSecBucket] bucket
** @param [r] pageno [ajlong] page number
**
** @return [void]
** @@
...
static EmbPBtpage btreeSplitSecLeaf(EmbPBtcache cache, EmbPBtpage spage)
{
    ...
    EmbPSecBucket *buckets = NULL;
    EmbPSecBucket cbucket  = NULL;
...
** @param [r] n [ajint] Number of IDs
**
** @return [EmbPSecBucket] initialised disc block cache structure
** @@
******************************************************************************/
...
** Delete a keyword primary bucket object
**
** @param [w] thys [EmbPSecBucket*] bucket
**
** @return [void]
** @@
******************************************************************************/
...
void embBtreeInsertSecId(EmbPBtcache cache, const AjPStr id)
{
    ...
    const char *ckey      = NULL;
    EmbPSecBucket lbucket = NULL;
    EmbPSecBucket rbucket = NULL;
    ajlong lblockno = 0L;
    ajlong rblockno = 0L;
    ajlong blockno  = 0L;
    ajlong shift    = 0L;
    ...
    ajDebug("In embBtreeInsertSecId\n");


    /* Only insert an ID if it doesn't exist */
    exists = embBtreeSecFromId(cache,MAJSTRSTR(id));
    if(exists)
    {
    ...
    ckey = ajStrStr(key);
    ajDebug("ckey = %s\n",ckey);
    spage = embBtreeSecFindInsert(cache,ckey);
    buf = spage->buf;
...
AjBool embBtreeSecFromId(EmbPBtcache cache, const char *key)
{
    ...
    ajDebug("In embBtreeSecFromId %d\n",cache->count);

    if(!cache->count)
	return ajFalse;
    
    page = embBtreeSecFindInsert(cache,key);
    buf = page->buf;
    GBT_NKEYS(buf,&nkeys);
    if(!nkeys)
	return ajFalse;
...
EmbPBtpage embBtreeSecFindInsert(EmbPBtcache cache, const char *key)
{
    ...
    ajDebug("In embBtreeSecFindInsert\n");
...
**
** @param [u] cache [EmbPBtcache] cache
** @param [r] pri [EmbPBtPri] keyword/id
**
** @return [void]
** @@
******************************************************************************/
...
AjPList embBtreeSecLeafList(EmbPBtcache cache, ajlong rootblock)
{
    ...
    page = embBtreeCacheRead(cache,rootblock);
    buf = page->buf;
	...
	if(cache->slevel)
	{
	    ...
	    if(right)
	    {
		page = embBtreeCacheRead(cache,right);	    
		buf = page->buf;
		btreeGetKeys(cache,buf,&karray,&parray);
	    }
	}
...
AjBool embBtreeVerifyId(EmbPBtcache cache, ajlong rootblock, char *id)
{
    ...
    spage = embBtreeSecFindInsert(cache,id);
    if(!spage)
	return ajFalse;
