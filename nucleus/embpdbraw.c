...
** @source embpdbraw.c
**
** Data structures and algorithms reading and writing PDB format files.
...
**
** EmbPElement is implemented as a pointer to a C data structure.
**
** @alias EmbSElement
** @alias EmbOElement
**
...
**
** EmbPElements is implemented as a pointer to a C data structure.
**
** @alias EmbSElements
** @alias EmbOElements
**
...
** @@
****************************************************************************/
typedef struct EmbSElements
{
    ajint      n;            
    EmbPElement *elms;        
}
EmbOElements, *EmbPElements;
...
**
** EmbPPdbfile is implemented as a pointer to a C data structure.
**
** @alias EmbSPdbfile
** @alias EmbOPdbfile
**
...
/*THIS_DIAGNOSTIC   static void diagnostic(EmbPPdbfile *pdbfile, ajint n); */
static void         PdbfileDel(EmbPPdbfile *pthis);
static EmbPPdbfile  ReadLines(AjPFile inf);
static EmbPPdbfile  PdbfileNew(ajint nlines, ajint nchains);
static AjBool       FirstPass(EmbPPdbfile *pdbfile, AjPFile logf, 
			      EmbPElements *elms, AjBool camask);
static AjBool       CheckChains(EmbPPdbfile *pdbfile, AjPFile logf, 
				ajint min_chain_size);
static AjBool       SeqresToSequence(AjPStr seqres, AjPStr *seq, 
				     AjBool camask, ajint *len);
static AjBool       CheckTer(EmbPPdbfile *pdbfile, AjPFile logf);
static AjBool       NumberChains(EmbPPdbfile *pdbfile, AjPFile logf);
static AjBool       MaskChains(EmbPPdbfile *pdbfile, AjPFile logf, 
			       ajint min_chain_size, AjBool camask, 
			       AjBool camask1, AjBool atommask);
static AjBool       StandardiseNumbering(EmbPPdbfile *pdbfile, 
					 AjPFile logf);
static AjBool       AlignNumbering(EmbPPdbfile *pdbfile, AjPFile logf, 
				   ajint lim);
static AjBool       PdbfileToPdb(AjPPdb *ret, EmbPPdbfile pdb);
static ajint        PdbfileFindLine(EmbPPdbfile pdb, ajint chn, 
				    ajint which, ajint pos);

/* Functions for Element object */
static EmbPElements ElementsNew(ajint nchains);
static void         ElementsDel(EmbPElements *ptr);
static EmbPElement  ElementNew(void);
static void         ElementDel(EmbPElement *ptr);
static AjBool       PdbfileChain(char id, EmbPPdbfile pdb, ajint *chn);
static AjBool       WriteElementData(EmbPPdbfile *pdbfile, AjPFile logf, 
				     EmbPElements elms);


/* These functions are called by embPdbWriteDomainRecordRaw */
...
/* These functions are called by embPdbWriteRecordRaw */
...
**
** @return [EmbPPdbfile] Pdbfile object pointer, or NULL on failure.
** @@
****************************************************************************/
static EmbPPdbfile ReadLines(AjPFile inf)
{
    EmbPPdbfile ptr    =NULL;		/* pdbfile object to be returned */
...
**
** @param [w] pthis [EmbPPdbfile*] Pdbfile object pointer
**
** @return [void]
** @@
****************************************************************************/
...
**
** @return [EmbPElements] Pointer to Elements object, or NULL on failure.
** @@
****************************************************************************/
static EmbPElements ElementsNew(ajint nelms)
{
    EmbPElements ret=NULL;
    ajint       x  =0;
...
**
** @param [w] ptr [EmbPElements*] Elements object pointer
**
** @return [void]
** @@
****************************************************************************/
...
**
** @return [EmbPElement] Pointer to Element object, or NULL on failure.
** @@
****************************************************************************/
static EmbPElement ElementNew(void)
{
...
**
** @param [w] ptr [EmbPElement*] Element object pointer
**
** @return [void]
** @@
****************************************************************************/
...
**
** @return [EmbPPdbfile] Pointer to pdbfile object, or NULL on failure.
** @@
****************************************************************************/
static EmbPPdbfile PdbfileNew(ajint nlines, ajint nchains)
{
    EmbPPdbfile ret  =NULL;
    ajint i=0;
...
** determined.
** Secondary structure information is also parsed and an EmbPElements 
** object is written.
** 
...
**
**
** @param [w] pdbfile  [EmbPPdbfile*]  Pdbfile object pointer
** @param [r] logf     [AjPFile]       Pointer to log file (build diagnostics)
** @param [W] elms     [EmbPElements*] Elements object pointer    
...
** @@
****************************************************************************/
static AjBool FirstPass(EmbPPdbfile *pdbfile, AjPFile logf, EmbPElements *elms, 
			AjBool camask)
{
    ...
    AjPList  listelms=NULL;	       /* Temp. list of secondary structure 
				          ...
				          records) */
    EmbPElement  elm=NULL;	       /* Temp. Element object pointer */
    EmbPElement  FirstStrand=NULL;     /* Temp. pointer to first strand of 
				          each sheet*/
...
** @@
****************************************************************************/
static AjBool StandardiseNumbering(EmbPPdbfile *pdbfile, AjPFile logf)
{	
    ajint   i=0;
...
****************************************************************************/
static AjBool AlignNumbering(EmbPPdbfile *pdbfile, AjPFile logf, ajint lim)
{
...
** 
**
** @param [r] pdbfile [EmbPPdbfile*]  Pdbfile object pointer
...
****************************************************************************/
/*THIS_DIAGNOSTIC 
static void diagnostic(EmbPPdbfile *pdbfile, ajint n)
{
    ajint i;
    i=0;
...
** @param [r] pdb     [EmbPPdbfile]  Pdbfile object pointer
** 
** @return [AjBool]  True on success, False otherwise
** @@
****************************************************************************/
...
** @@
****************************************************************************/
static ajint  PdbfileFindLine(EmbPPdbfile pdb, ajint chn, ajint which, 
			      ajint pos)
{
    ajint a=0;
    /* a will give the number of the first coordinate line 
...
** @param [r] pdb [EmbPPdbfile] Pdbfile object
...
** @param [W] pdbfile [EmbPPdbfile *] Pdbfile object pointer
** @param [r] logf    [AjPFile      ] Pointer to log file (build diagnostics)
** @param [r] elms    [EmbPElements ] Elements object pointer
** 
** @return [AjBool]  True on success, False otherwise
** @@
****************************************************************************/
...
{
    EmbPPdbfile pdbfile      =NULL; /* Pdbfile structure (for raw data)    */
    AjPPdb     ret          =NULL;  /* Pdb structure (for parsed data)     */
    EmbPElements elms=NULL;         /* Elements structure (for parsed data)*/
    


    if(!inf || !logf)
    {
	ajWarn("Null arg passed to embPdbReadRawNew");
	return NULL;
    }
...
/* @func embPdbWriteDomainRecordRaw *****************************************
**
				   ...
				   AjPScop scop, AjPFile outf, AjPFile errf)
{
    /* Check args */
    if(!outf || !scop)
	ajFatal("Invalid args passed to embPdbWriteDomainRecordRaw");
	...
	ajFatal("Invalid mode in embPdbWriteDomainRecordRaw");
...
/**************************************************************/
/* The following functions are called by embPdbWriteRecordRaw */
/**************************************************************/
			    ...
			    ajint chn, AjPFile outf, AjPFile errf)
{
    /* Check args */
    if(!outf || !pdb)
	ajFatal("Invalid args passed to embPdbWriteRecordRaw");
	...
	ajFatal("Invalid mode in embPdbWriteRecordRaw");
...
AjBool embPdbWriteAllRaw(ajint mode, AjPPdb pdb, AjPFile outf, AjPFile errf)
{
    ajint x;
    ajint y;
    ...
    embPdbWriteRecordRaw(ajHEADER, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajTITLE, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajCOMPND, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajSOURCE, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajEMPTYREMARK, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajRESOLUTION, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajEMPTYREMARK, pdb, 0, 0, outf, NULL);
    
    
    /* Write SEQRES records */
    for(x=0;x<pdb->Nchn;x++)
	if(!embPdbWriteRecordRaw(ajSEQRES_CHAIN, pdb, 0, x+1, outf, errf))
	{
	    ajWarn("Error writing file in embPdbWriteAllRaw"); 
	    return ajFalse;
	}
	...
	for(x=0;x<pdb->Nchn;x++)
	{
	    if(mode == ajPDB)
	    {
		if(!embPdbWriteRecordRaw(ajATOMPDB_CHAIN, pdb, y+1, x+1, 
					 outf, NULL))
		{
		    ajWarn("Error writing file in embPdbWriteAllRaw"); 
		    return ajFalse;
	    ...
	    {
		if(!embPdbWriteRecordRaw(ajATOMIDX_CHAIN, pdb, y+1, x+1, 
					 outf, NULL))
		{
		    ajWarn("Error writing file in embPdbWriteAllRaw"); 
		    return ajFalse;
		...
		ajFatal("Invalid mode in embPdbWriteAllRaw");
	    
	
	    if(!embPdbWriteRecordRaw(ajHETEROGEN, pdb, y+1, 0, outf, NULL))
	    {
		ajWarn("Error writing file in embPdbWriteAllRaw"); 
		return ajFalse;
	    }
			    ...
			    AjPFile outf, AjPFile errf)
{
	...
	if(!ajPdbChnidToNum(scop->Chain[z], pdb, &chn))
	{
	    ajWarn("Chain incompatibility error in "
		   "embPdbWriteDomainRaw");			
		
	    ajFmtPrintF(errf, "//\n%S\nERROR Chain incompatibility error "
			"in embPdbWriteDomainRaw\n", scop->Entry);
    ...
    /* Write bibliographic info. */
    embPdbWriteDomainRecordRaw(ajHEADER_DOMAIN, NULL, 0, scop, outf, NULL);
    embPdbWriteRecordRaw(ajTITLE, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajCOMPND, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajSOURCE, pdb, 0, 0, outf, NULL);
    embPdbWriteRecordRaw(ajEMPTYREMARK, pdb, 0, 0, outf, NULL);
    ...
    /* Write SEQRES records */
    if(!embPdbWriteDomainRecordRaw(ajSEQRES_DOMAIN, pdb, 0, scop, outf, 
				   errf))
    {
	ajWarn("Error writing file in embPdbWriteDomainRaw"); 
	return ajFalse;
    } 
