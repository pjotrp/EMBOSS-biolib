...
static AjBool      feattableWriteUnknown(AjPFeattabOut ftout,
                                    ...
                                    const AjPFeattable thys)
{
    ...
    if(!ftout || !ftout->Handle)
	return ajFalse;
    ajFmtPrintF(ftout->Handle, "Unknown feature format hence no output."
		"Except this line!!\n");
				    ...
				    if(tablestr)
                                    {
                                        if(recursion)
                                        {
                                            ajDebug("ERROR:%S duplicate localname type "
                                                  "%S='%S' already defined as '%S'\n",
                                                  fname, localname,
                                                  sofaname, tablestr);
	...
	if(tablestr)
        {
            if(recursion)
            {
                ajDebug("ERROR: %S: duplicate type %S='%S' already defined as '%S'\n",
                        fname, savetype, typtagstr, tablestr); 
