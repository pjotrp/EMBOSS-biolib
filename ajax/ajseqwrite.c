...
*/
    ...
    if(outseq->Accuracy)
    {
        ...
        for(j=0;j<ilen;j++)
        {
            ...
            /*adjust low phred score to maximum 0.75 error rate = random */
            if(pval > 0.75)
                pval = 0.75;
            ...
            if(qchar > 126)
                ...
                qchar = 33;
            ajStrAppendK(&seq, (char) qchar);
            ajDebug("[%d] aval:%.4f pval: %.4f qval:%.4f %d '%c'\n",
                    j, sval, pval, qval, (int)qval, qchar);
        }
        ajFmtPrintF(outseq->File, "%S\n", seq);
    }
