 ...
 *
 * You may contact the author by email: ptribble@hgmp.mrc.ac.uk
 *
 * EmbreoStartup puts up a startup banner, or splash screen, with a login
 * prompt if necessary, and provides a mechanism to monitor the startup
 * of the application and take corrective measures if problems occur.
 *
...
public class EmbreoStartup {
   ...
   */
  public EmbreoStartup(final EmbreoParams mysettings, int iprogressmax) {
    ...
    // try to find a splash icon
    ClassLoader cl = this.getClass().getClassLoader();
    URL splashIconURL;
    splashIconURL = cl.getResource("resources/"+appName+"_splash.png");
    if (splashIconURL == null) {
      splashIconURL = cl.getResource("resources/"+appName+"_splash.gif");
    }
    if (splashIconURL == null) {
      splashIconURL = cl.getResource("resources/embreo_splash.png");
    }
    if (splashIconURL == null) {
      splashIconURL = cl.getResource("resources/embreo_splash.gif");
    }
    // create a splash label
    JLabel splashl;
    ImageIcon splashi;
    JPanel logoPanel = new JPanel(new BorderLayout());
    if (splashIconURL != null ) {
      splashi = new ImageIcon(splashIconURL);
      splashl = new JLabel(splashi);
      logoPanel.setSize(splashi.getIconWidth()+12,splashi.getIconHeight()+12);
    } else {
      splashl = new JLabel("EMBREO");
      logoPanel.setSize(200,100);
    }
    logoPanel.add(splashl);
    ...
    splashp.setBorder(BorderFactory.createMatteBorder(4,4,4,4,Color.black));

    splashf.getContentPane().add(splashp);
    splashf.setSize(480,360);

    // all added, display the frame
    EmbreoUtils.CenterFrame(splashf);
    //EmbreoUtils.CenterWindow(splashf);
